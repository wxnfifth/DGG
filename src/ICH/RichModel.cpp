...
void CRichModel::AssignColor(const vector<double>& diffused_value)
{
    m_vertex_colors.resize( m_Verts.size());
    assert(diffused_value.size() == m_vertex_colors.size());
    for(int i = 0; i < m_vertex_colors.size();++i) {
        m_vertex_colors[i] = color_red * diffused_value[i];
    }
}
